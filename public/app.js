  ...
  state.links.forEach(link => {
    ...
    li.innerHTML = `
      ...
      <div class="link-right">
        ${state.authed ? '<button class="btn small edit-btn">编辑</button>' : ''}
        <a class="btn small" href="${link.url}" target="_blank" rel="noopener">打开</a>
      </div>`;

    if(state.authed){
      $('.edit-btn', li).addEventListener('click', () => openLinkDialog(link));
      setupDragReorder(li);
    }
...
function setupDragReorder(el){
  let pressTimer;
  el.addEventListener('touchstart', () => { pressTimer = setTimeout(()=> el.draggable = true, 300); }, {passive:true});
  el.addEventListener('touchend', () => { clearTimeout(pressTimer); setTimeout(()=> el.draggable=false, 0); });
...
async function persistOrder(){
  ...
  await api('/api/links', { method:'PATCH', body: JSON.stringify({ order: ids }) });
  await load();
}
...
$('#logout-btn').addEventListener('click', async ()=>{ await api('/api/logout', { method:'POST' }); await load(); });
...
$('#edit-profile').addEventListener('click', ()=>{
  ...
  $('input[name="backgroundUrl"]', d).value = state.profile.backgroundUrl || '';
  d.showModal();
});
...
function openLinkDialog(link={}){
  ...
  $('#delete-link').onclick = async ()=>{
    ...
    if(confirm('确定删除？')){
      await api('/api/links', { method:'DELETE', body: JSON.stringify({ id: link.id }) });
      d.close(); await load();
    }
  };
...
$('#link-form').addEventListener('submit', async (e)=>{
  ...
  const fd = new FormData(e.currentTarget);
  const payload = Object.fromEntries(fd);
  const body = JSON.stringify({ id: payload.id || undefined, icon: payload.icon, title: payload.title, url: payload.url });
  await api('/api/links', { method: payload.id ? 'PUT' : 'POST', body });
  $('#link-dialog').close();
  await load();
});
